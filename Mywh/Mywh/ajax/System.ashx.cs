    ...
    public class System : BaseAshx
    {
        ...
        private string userLogin()
        {
            ...
            try
            {
                ...
                strSql = @"select * from system_menu  
                           where id in(
                               select menuId from system_roleMenuAction where roleId = "+ dt.Rows[0]["roleId"] + @"
                               union
                               select menuId from system_userMenuAction where userId = " + dt.Rows[0]["userId"] + @" and hasPermissin = 1
                           )
                           and id not in(
                               select menuId from system_userMenuAction where userId = " + dt.Rows[0]["userId"] + @" and hasPermissin = 0
                           )";
                dt = SqlHelper.GetDataTable(strSql);
                
                string strMenu= MenuBuild(0,dt);
        ...
        private string deleteRole()
        {
            ...
            try
            {
                ...
                strSql = "delete from system_role where id=" + id;
                int count = SqlHelper.ExecuteNonQuery(strSql);
                if (count > 0)
                {
                    my.flag = 1;
